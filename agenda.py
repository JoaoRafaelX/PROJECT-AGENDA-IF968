...
def ajuda(ignorarComandos = ""):
    print("\n\nPROJECT AGENDA por André Loponte e João Rafael\n" +
          ...
          "Exemplos de operações: R 7 | F 4 | P 1 A | P 2 | L | L 3 | L 1 4 | L 21102015")
...
def priorizar(comandos):
    ...
    prioridade = ""
    if len(comandos) > 1:
        if len(comandos[1]) == 1 and comandos[1] >= "A" and comandos[1] <= "Z":
            prioridade = "(" + comandos[1] + ")"
        elif comandos[1][1] >= "A" and comandos[1][1] <= "Z":
...
def listar(comandos):
    ...
    if len(comandos) == 1:
        if soDigitos(comandos[0]):
            ...
            if dataValidacao(comandos[0]):
                for i in range(len(itens)):
                    dataI = itens[i][0]
                    dataI = dataI[0:2] + dataI[3:5] + dataI[6:]
                    if comandos[0] == dataI:
        ...
        elif comandos[0].upper() >= "A" and comandos[0].upper() <= "Z":
            for i in range(len(itens)):
                #Impedir que dê erro de index quando não tem prioridade
                if itens[i][2]:
                    if comandos[0].upper() == itens[i][2][1].upper():
                        printCores((str(i + 1) + ". " + mostrarItem(itens[i])), itens[i][2])
            return True
    ...
    #Caso não o comando não encaixe em nenhum dos casos passados...
...
def printCores(texto, prio, resetar = True):

    #São 5 cores específicas pras prioridades mais importantes
    #E um cinza mais escuro pra última prioridade
    switchCores = {"(A)":       "\033[1;31m",
                   "(B)":       "\033[1;32m",
                   "(C)":       "\033[1;33m",
                   "(D)":       "\033[1;36m",
                   "(E)":       "\033[1;34m",
